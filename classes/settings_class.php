...
class birthday_mail_settings{

	var $settings;
	var $option = 'bp-birthday-mail';

	public function __construct(){
		add_options_page(__('Birthday Mails ','bp-birthday-mail'),__('Birthday Mails','bp-birthday-mail'),'manage_options','bp-birthday-mail',array($this,'settings'));
	...
	function generate_form($tab,$settings=array()){
		...
		if(!empty($crons)){
			?>
			<script>
			var confirm_message_cron= "<?php echo __('Are you sure you want to remove this schedule?','bmbp'); ?>";
			...
			foreach($crons as $timestamp => $cron){
				if(isset($cron['bp_send_birthday_email']) || isset($cron['bp_send_birthday_email'])){
					$check_emails++;
						...
						$data = '<form method="post" id="drip" class="remove_schedule" action="?page=bp-birthday-mail"><input name="hook" value="bp_send_birthday_email" type="hidden"  data-hook="bp_send_birthday_email" >';
